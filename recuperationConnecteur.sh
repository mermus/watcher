...
recuperationConnecteur() {
   ...
   # CONNECTEUR=`awk '{ for(i=1;i<=NF;i++) { if($i!="kernel" && $i!="data1base" && $i!="data2base"&& $i!="tempo" && $i!="data5base"){ printf $i";"} } }' $PLTDIR/scripts/processList`
    CONNECTEUR=`awk '{ for(i=1;i<=NF;i++) { if($i!="kernel" && $i!="data1base" && $i!="data2base"&& $i!="tempo" && $i!="data5base"){ printf $i";"} } }' repaxigate/scripts/processList`
